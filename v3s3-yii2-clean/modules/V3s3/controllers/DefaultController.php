...
class DefaultController extends YII2_Controller {
	...
	public function actionPut() {
		...
		try {
			if (empty($name) || ($name == '/')) {
				throw new V3s3ControllerRequestValidationException(V3s3Module::t('V3s3', 'V3S3_EXCEPTION_CONTROLLER_REQUEST_VALIDATION_PUT_EMPTY_OBJECT_NAME'), V3s3ControllerRequestValidationException::PUT_EMPTY_OBJECT_NAME);
			} else if (strlen($name) > 1024) {
				throw new V3s3ControllerRequestValidationException(V3s3Module::t('V3s3', 'V3S3_EXCEPTION_CONTROLLER_REQUEST_VALIDATION_OBJECT_NAME_TOO_LONG'), V3s3ControllerRequestValidationException::OBJECT_NAME_TOO_LONG);
			}
	...
	public function actionDelete() {
		...
		try {
			if (empty($name) || ($name == '/')) {
				throw new V3s3ControllerRequestValidationException(V3s3Module::t('V3s3', 'V3S3_EXCEPTION_CONTROLLER_REQUEST_VALIDATION_DELETE_EMPTY_OBJECT_NAME'), V3s3ControllerRequestValidationException::DELETE_EMPTY_OBJECT_NAME);
			} else if (strlen($name) > 1024) {
				throw new V3s3ControllerRequestValidationException(V3s3Module::t('V3s3', 'V3S3_EXCEPTION_CONTROLLER_REQUEST_VALIDATION_OBJECT_NAME_TOO_LONG'), V3s3ControllerRequestValidationException::OBJECT_NAME_TOO_LONG);
			}
		} catch(V3s3ControllerRequestValidationException $e) {
	...
	public function actionPost() {
		...
		if(!empty($input) && empty($parsed_input)) {
			try {
				throw new V3s3ControllerRequestValidationException(V3s3Module::t('V3s3', 'V3S3_EXCEPTION_CONTROLLER_REQUEST_VALIDATION_POST_INVALID_REQUEST'), V3s3ControllerRequestValidationException::POST_INVALID_REQUEST);
			} catch(V3s3ControllerRequestValidationException $e) {
				YII2_Yii::$app->response->format = YII2_Response::FORMAT_JSON;
				return [
					'status'=>0,
