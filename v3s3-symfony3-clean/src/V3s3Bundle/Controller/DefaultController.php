...
use V3s3Bundle\Controller\Exception\V3s3ControllerRequestValidationException;
...
class DefaultController extends Controller {
	...
	public function putAction($name, Request $request) {
		...
		try {
			if (empty($name) || ($name == '/')) {
				throw new V3s3ControllerRequestValidationException(
					$this->get('translator')->trans('V3S3_EXCEPTION_CONTROLLER_REQUEST_VALIDATION_PUT_EMPTY_OBJECT_NAME', [], self::TRANSLATION_DOMAIN),
					V3s3ControllerRequestValidationException::PUT_EMPTY_OBJECT_NAME
				);
			} else if (strlen($name) > 1024) {
				throw new V3s3ControllerRequestValidationException(
					$this->get('translator')->trans('V3S3_EXCEPTION_CONTROLLER_REQUEST_VALIDATION_OBJECT_NAME_TOO_LONG', [], self::TRANSLATION_DOMAIN),
					V3s3ControllerRequestValidationException::OBJECT_NAME_TOO_LONG
				);
			}
	...
	public function getAction($name, Request $request) {
		...
		try {
			if (empty($name) || ($name == '/')) {
				throw new V3s3ControllerRequestValidationException(
					$this->get('translator')->trans('V3S3_EXCEPTION_CONTROLLER_REQUEST_VALIDATION_GET_EMPTY_OBJECT_NAME', [], self::TRANSLATION_DOMAIN),
					V3s3ControllerRequestValidationException::PUT_EMPTY_OBJECT_NAME
				);
			} else if (strlen($name) > 1024) {
				throw new V3s3ControllerRequestValidationException(
	...
	//  5.1. SET SINGLE RESPONSE HEADER. RETURN JSON RESPONSE. (return a v3s3-object-id (ID of the deleted entity) header and a JSON response)
	// {else}
	//  5.2. SET RESPONSE STATUS. RETURN JSON RESPONSE. (return 404 status and a JSON response)
	// {endif}
	public function deleteAction($name, Request $request) {
		...
		try {
			if (empty($name) || ($name == '/')) {
				throw new V3s3ControllerRequestValidationException(
					$this->get('translator')->trans('V3S3_EXCEPTION_CONTROLLER_REQUEST_VALIDATION_DELETE_EMPTY_OBJECT_NAME', [], self::TRANSLATION_DOMAIN),
					V3s3ControllerRequestValidationException::PUT_EMPTY_OBJECT_NAME
				);
			} else if (strlen($name) > 1024) {
				throw new V3s3ControllerRequestValidationException(
	...
	public function postAction($name, Request $request) {
		...
		if(!empty($input) && empty($parsed_input)) {
			try {
				throw new V3s3ControllerRequestValidationException(
					$this->get('translator')->trans('V3S3_EXCEPTION_CONTROLLER_REQUEST_VALIDATION_POST_INVALID_REQUEST', [], self::TRANSLATION_DOMAIN),
					V3s3ControllerRequestValidationException::POST_INVALID_REQUEST
				);
			} catch(V3s3ControllerRequestValidationException $e) {
				return new JsonResponse(
					[
